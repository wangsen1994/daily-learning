...
struct RaftNode{
	...
	uint64_t select_timeout_due = 0;		 // At this point the Follower start selection
	uint64_t selection_fail_timeout_due = 0; // At this point the Candidate's selection timeout
	...
	uint64_t start_timeout;   // Reject RPC from previous test cases.
	uint64_t last_timeout;    // TODO Reject RPC from previous meeage from current Leader.
	uint64_t last_seq;

	#if defined(USE_GRPC_ASYNC)&& !defined(USE_GRPC_STREAM)&&!defined(USE_GRPC_SYNC_BARE)
	...
	void print_state(std::lock_guard<std::mutex>&guard){
		printf("%15s %12s %5s %9s %7s %6s %6s %6s\n", 
